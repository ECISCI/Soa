 ...
 * @author Ming
 * @描述 手把手教你nginx
 * @源码包 zzlinuxinstallpackage目录下nginx-1.8.0.tar.gz
 */
public class Nginx {
    /**
     * @1.nginx简介
     ...
     *
     * yum命令 yum install -y openssl openssl-devel
     *
     ...
     * 6.3 编译nginx
     *
     * 执行config命令 config命令在 nginx_config.text文件中
     *
     * 6.4### ！！！需要注意
     * nginx_config.text 命令中有一条 prefix=/usr/local/nginx （这是指定nginx最终的安装目录,无需手动创建）
     *
     * 如果该目录下有nginx文件夹,请将该文件夹删除
     *
     * 6.5 执行config命令之后会在nginx目录下出现makefile可编译文件,这样就可以执行make命令了
     *
     * 7.编译
     *
     * 命令 make
     ...
     */
    /*
     * @1 进入nginx安装目录 , 命令 cd /usr/local/nginx
     *
     * @2 进入sbin目录 命令 cd /sbin
     *
     * @3 启动nginx ./nginx
     ...
     *
     * ！！！注意:外部浏览器访问的时候一定要先关闭防火墙 service iptables stop
     * */
    ...
    /*@1 进入nginx的配置文件目录
     *
     *进入nginx的安装目录 命令 cd /usr/local/nginx/conf
     *
     *@2 查看配置文件
     *
     * 命令 vim nginx.conf
